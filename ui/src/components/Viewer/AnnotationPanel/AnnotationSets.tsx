...
import type { AnnotationSetsDetails } from "../../../types/Viewer";
...
function AnnotationSets({
  ...
  annotationSets: AnnotationSetsDetails | null;
  ...
  const [annotationSetIdLoadingMap, setAnnotationSetIdLoadingMap] = useState<
    Record<string, boolean>
  >({});
  const [annotationSetContextMenuId, setAnnotationSetContextMenuId] = useState<
    string | null
  >(null);

  const handleDownloadAnnotationSetClick = async (annotationSetId: string) => {
    setAnnotationSetIdLoadingMap((prev) => ({
      ...prev,
      [annotationSetId]: true,
    }));
    await handleDownloadAnnotationSet(annotationSetId);
    setAnnotationSetIdLoadingMap((prev) => ({
      ...prev,
      [annotationSetId]: false,
    }));
  };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="grow rounded-lg overflow-y-auto p-2">
        {annotationSets?.annotation_sets.length === 0 ? (
          <div className="flex min-h-[10rem] flex-col items-center justify-center gap-2 px-4 py-10 text-center">
            ...
            <p className="max-w-[16rem] text-xs leading-relaxed text-[var(--muted)]">
              Create annotations in the viewer, then save — your sets will
              appear here.
            </p>
          </div>
        ) : (
          annotationSets?.annotation_sets.map((annotationSet) => {
            ...
            return (
              ...
              >
                ...
                <div className="shrink-0 self-stretch rounded-r-lg overflow-hidden border-l border-[var(--border)]/60">
                  ...
                  >
                    <CloudDownIcon className="size-4 shrink-0" />
                  </button>
                </div>
                {annotationSetContextMenuId === idStr && (
                  <>
                    <Backdrop
                      onClick={() => setAnnotationSetContextMenuId(null)}
                    />
                    <div
                      role="menu"
